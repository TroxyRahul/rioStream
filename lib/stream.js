...
export default async function getStreamData(fullPath) {
  ...
  const configMatch = scriptContent?.match(/(let pc|new HDVBPlayer)\(\s*(\{[\s\S]*?\})\s*\)/);
  if (!configMatch) throw new Error("Player config not found in HTML");

  let pc;
  try {
    pc = JSON.parse(configMatch[2]);
  } catch (err) {
  ...
  if (isTV) {
    ...
    for (const season of seriesJson || []) {
      ...
      for (const episode of season.folder || []) {
        if (episodeParam && episodeParam !== episode.episode?.toString()) continue;
